...
function ensureResourceAvailable(sessionConfig, path, resource) {
    if (resource) {
        sessionConfig.addResource(path, resource);
    }

    if (!sessionConfig.getResource(path)) {
...
module.exports = {
    ...
    load: function (path, resource) {
        ensureResourceAvailable(this, path, resource);
        this.loadResources.push(normalizedPath(path));
    },

    loadFirst: function (path, resource) {
        ensureResourceAvailable(this, path, resource);
        this.loadResources.unshift(normalizedPath(path));
    },
    ...
    get loadResources () {
        if (!this._loadResources) this._loadResources = [];
