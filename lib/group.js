            ...
            (function (self, globPromise, resource) {
                ...
                glob(absolutePath, function (err, matches) {
                    if (err) {
                        ...
                        addResources.call(self, matches, resource);
                        globPromise.resolve();
                    }
                });
...
function addResources(paths, baseResource) {
    for (var i = 0, ii = paths.length; i < ii; i++) {
        var path = paths[i];

        var resource = {};
        resource.path = shortenedPath.call(this, path);
        if ("headers" in baseResource) {
            resource.headers = baseResource.headers;
        }
        this.resourceSet.addFile(path, resource);
    }
}