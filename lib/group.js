...
module.exports = {
    create: function (options, rootPath) {
        ...
        return buster.extend(Object.create(this), {
            ...
            rootPath: options.rootPath || rootPath,
    ...
    resolve: function () {
        ...
        var resourcePromises = loadResources.call(this, this.resources);

        var promise = bPromise.create();

        var load = [].concat(this.load);
        for (var i = 0, ii = LOAD_ALIASES.length; i < ii; i++) {
            var alias = LOAD_ALIASES[i];
            if (this[alias] instanceof Array) {
                load = load.concat(this[alias]);
            }
        }
...
function extractOptions(options) {
    for (var i = 0, l = CONFIG_OPTIONS.length, opt = {}; i < l; ++i) {
        opt[CONFIG_OPTIONS[i]] = options[CONFIG_OPTIONS[i]];
    }
