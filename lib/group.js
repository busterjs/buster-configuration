...
function extractExtensions(config) {
    return (config.extensions || []).map(function (ext) {
        var name = ext.name || ext;
        var extConf = config[name] || {};
        delete config[name];
        return { name: name, config: extConf, extension: ext.extension };
    });
}
...
function loadExtension(ext) {
    if (!ext.name && typeof ext !== "string") { return ext; }
    var module = ext.extension || moduleLoader.load(ext.name);
    ...
    bundleFramework: function () {
        this.on("load:framework", function (rs) {
            ...
            when.all([
                ...
                var iife = br.processors.iife(["buster", "sinon"]);
                rs.get(bundleResourceName).addProcessor(iife);
            }.bind(this));
        }.bind(this));
        return this;
    },
