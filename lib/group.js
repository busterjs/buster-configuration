...
var cg = module.exports = B.extend(B.eventEmitter.create(), {
    ...
    setupFrameworkResources: function () {
        ...
        this.resourceSet.addResource(compatResourceName, {
            ...
            this.resourceSet.loadPath.prepend(compatResourceName);
        }.bind(this));
        ...
        this.resourceSet.addResource(bundleResourceName, {
            ...
            this.resourceSet.loadPath.prepend(bundleResourceName);
        }.bind(this));
    ...
    rs.addResources(group.resources).then(function () {
        B.series([
            ...
            when.all(results).then(function () {
                rs.concat.apply(rs, results).then(B.partial(done, null), done);
            }, done);
        });
    }, function (err) {
        if (err) { done(err); }
...
function resolveModules(modules) {
    ...
    for (i = 0, l = modules.length; i < l; i++) {
        ...
        this.resourceSet.addFileResource(absolutePath, {path: resourcePath});
