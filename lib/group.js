...
module.exports = {
    create: function (groupData, rootPath) {
        ...
        instance.groupData = initializeData(groupData);
        instance.environment = groupData.environment || groupData.env || "browser";
        instance.rootPath = extractRootPath(groupData, rootPath);
        instance.server = extractServer(groupData);
        extractOptions(instance, groupData);
    ...
    resolve: function () {
        ...
        var load = [].concat(this.groupData.load);
        for (var i = 0, ii = LOAD_ALIASES.length; i < ii; i++) {
            var alias = LOAD_ALIASES[i];
            if (this.groupData[alias] instanceof Array) {
                load = load.concat(this.groupData[alias]);
            }
        }

        resourcePromises.push(addLoadResources.call(this, load));
...
function addLoadResources(loadResources) {
    var promise = bPromise.create();
...
function addLoadResourcesIter(loadResources, promise) {
    ...
    if (loadResources.length == 0) {
        ...
        addLoadResource.call(this, loadResources.shift()).then(function () {
            addLoadResourcesIter.call(self, loadResources, promise);
        }, function (err) {
            promise.reject(err);
        });
    }
...
function extractRootPath(groupData, defaultRoot) {
    if ("rootPath" in groupData) {
        var rootPath = groupData.rootPath;
        delete groupData.rootPath;
        return rootPath;
    }
...
function extractServer(groupData) {
...
function extractOptions(group, data) {
    group.options = group.options || {};

    for (var i = 0, l = CONFIG_OPTIONS.length; i < l; ++i) {
        group.options[CONFIG_OPTIONS[i]] = data[CONFIG_OPTIONS[i]];
    }
}

function initializeData(groupData) {
    var data = groupData || {};
